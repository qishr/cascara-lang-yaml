...
class StreamingFlowParserTests {
    ...
    void testEmptyFlowContainers() {
        ...
        assertTrue(emptyMapRoot instanceof YamlMap);
        assertTrue(((YamlMap) emptyMapRoot).isEmpty());

        // Test empty flow sequence
        YamlNode emptySeqRoot = parser.parse("[]");
        assertTrue(emptySeqRoot instanceof YamlSequence);
        assertEquals(0, ((YamlSequence) emptySeqRoot).size());
    }
    ...
    void testStandardFlowSequence() {
        ...
        YamlSequence seq = (YamlSequence) root;

        assertEquals(3, seq.size());
        assertEquals("macos", ((YamlScalar) seq.get(0)).asString());
        assertEquals("windows", ((YamlScalar) seq.get(1)).asString());
        assertEquals("linux", ((YamlScalar) seq.get(2)).asString());
    }
    ...
    void testNestedFlowContainers() {
        ...
        assertTrue(deps instanceof YamlSequence);
        assertEquals(2, ((YamlSequence) deps).size());
    }
}